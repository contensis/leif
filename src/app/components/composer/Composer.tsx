    ...
    switch (field.type) {
      ...
      case 'image': {
        return (
          <Region key={`region-${idx}`} width="medium" margin="medium">
            <ImageBlock
              image={field.value}
              title={field.value.altText}
              text={field.value.caption}
              key={idx}
            />
          </Region>
        );
      ...
      case 'featuredProduct': {
        const linkObject = {
          ...
          href:
            field.value.product && field.value.product.sys.uri
              ? field.value.product.sys.uri
              : (!field.value.product || !field.value.product.sys.uri) &&
                field.value.externalButtonLink
              ? field.value.externalButtonLink
              : '/shop',
        };
        return (
      ...
      case 'video': {
        return (
          <Region key={`region-${idx}`} width="medium" margin="large">
            <VideoPlayer
              title={field.value.entryTitle}
              hasControls={true}
              type={field.value.source}
              externalURL={field.value.externalURL}
              internalVideo={
                field.value.internalVideo &&
                field.value.internalVideo.asset.sys.uri
              }
              key={idx}
            />
          </Region>
